...
bool Desktop::remove()
{
    // At least 2 workspaces
    if (workspaces_.size() <= 2) { return false; }
...
//-----------------------------------------------------------------------------
// Thread functions (static) for [create], [remove] and [jumpto].

unsigned int __stdcall Desktop::createThread(void *arg)
{
    ((Desktop *)arg)->__create();
    return 0;
}

unsigned int __stdcall Desktop::removeThread(void *arg)
{
    ((Desktop *)arg)->__remove();
    return 0;
}

unsigned int __stdcall Desktop::jumptoThread(void *arg)
{
